...
function writeInDesc(tab){
    ...
    if(tab.otype == "file"){
		...
		for(var i = 0; i < tab.length; i++){
			...
			if(tab[i].otype == "class"){
				...
				if(tab[i].parameters != undefined){
					...
					for(var param of tab[i].parameters){
						...
						if(param.type.id){
							oname += '<a href="files.html?=' + param.type.id + '">' + param.type.name;
							oname += "</a> ";
						}else{
							oname += param.type.name + " ";
						}
				...
				if(tab[i].type.id){
					oname += '<a href="files.html?=' + tab[i].type.id + '">' + tab[i].type.name + "</a> ";
				}else{
					oname += tab[i].type.name + " ";
				}
				if(tab[i].mods != undefined){
					for(var mod of param.mods){
						if(mod.keyword){
			...
			descStr += '<li><span class="likeH" id="' + tab[i].id + '"><code>' + oname + '</code></span>' + (tab[i].inherited ? '<br/>Inherits from <code>' + tab[i].inherits + '</code><br/>' : "") + '<p>';
